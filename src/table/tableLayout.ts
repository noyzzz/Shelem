...
import type { TableViewModel } from "./tableTypes";
import { SEAT_ANCHORS, TABLE_SURFACE_Y } from "./tableConfig";
      ...
      target: {
        ...
        quaternion: faceUp(
          new Euler(
            LOCAL_HAND_TILT,
            0,
            MathUtils.degToRad(-distance * 2.2),
          ),
        ),
...
function layoutOpponentHands(model: TableViewModel): DesiredCard[] {
  ...
  model.seats.forEach((seat) => {
    if (!seat.player || seat.displayPosition === "south" || seat.handCount === 0) {
      return;
    }
    ...
    for (let index = 0; index < shown; index += 1) {
      ...
      if (seat.displayPosition === "north") {
        position = new Vector3(spread, TABLE_SURFACE_Y + 0.1 + index * 0.003, -2.7);
        rotation = new Euler(-Math.PI / 2, 0, MathUtils.degToRad(spread * 18));
      } else if (seat.displayPosition === "west") {
        position = new Vector3(-4.15, TABLE_SURFACE_Y + 0.1 + index * 0.003, spread);
